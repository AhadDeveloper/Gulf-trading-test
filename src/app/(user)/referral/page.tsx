...
export default function ReferralLinkPage() {
  ...
  useEffect(() => {
    const fetchReferral = async () => {
      ...
      // Build referral link
      const link = `${window.location.origin}/signup?ref=${info.referralCode}`;
      setReferralUrl(link);

      // TODO: Compute level1 and level2 data based on your referral structure
      // Example placeholder: replace with actual DB query for team/commission/investment
      const levels: ReferralData = {
        level1: { team: 0, commission: "Rs0", investment: "Rs0" },
        level2: { team: 0, commission: "Rs0", investment: "Rs0" },
      };

      setReferralData(levels);
    };
